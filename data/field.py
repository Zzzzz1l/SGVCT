...
class ImageAllFieldWithMask(RawField):
    ...
    def preprocess(self, x, avoid_precomp=False):
        sg_root = "../../lab/Parallel/data/features/coco_pred_sg"
        sg_dict_path = "../../lab/Parallel/data/features/coco_pred_sg_rela.npy"
        image_id = int(x.split('_')[-1].split('.')[0])
        ...
        try:
            ...
            for i, obj, attr in sg_attr:
                obj = sg_i2w[obj]
                sg_obj.append(obj)
            sg_obj = list(set(sg_obj))
            interest = " ".join(sg_obj)
        ...
        return precomp_data.astype(np.float32), relative_boxes.astype(np.float32), grid_data.astype(np.float32), mask.astype(np.float32),interest
