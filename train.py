...
from data import ImageDetectionsField, TextField, RawField, ImageAllFieldWithMask
...
def evaluate_loss(model, dataloader, poly_loss, text_field):
    ...
    with tqdm(desc='Epoch %d - validation' % e, unit='it', total=len(dataloader)) as pbar:
        for it, (detections, boxes, grids, masks,interests ,captions) in enumerate(dataloader):
                ...
                out = model(detections, boxes, grids, masks, captions, interests)
...
def evaluate_metrics(model, dataloader, text_field):
    ...
    with tqdm(desc='Epoch %d - evaluation' % e, unit='it', total=len(dataloader)) as pbar:
        ...
        for it, ((images, boxes, grids, masks, interests), caps_gt) in enumerate(iter(dataloader)): 
            ...
            with torch.no_grad():
                out, _ = model.beam_search(images, 20, text_field.vocab.stoi['<eos>'], 5, out_size=1,
                                           **{'boxes': boxes, 'grids': grids, 'masks': masks,'interests':interests}) 
...
def train_scst(model, dataloader, optim, cider, text_field):
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader)) as pbar: 
        for it, ((detections, boxes, grids, masks,interests), caps_gt) in enumerate(dataloader):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='ZH3')
    ...
    parser.add_argument('--xe_least', type=int, default=16)
    parser.add_argument('--xe_most', type=int, default=16)
    parser.add_argument('--refine_epoch_rl', type=int, default=17)
    ...
    # Pipeline for image regions and grids and mask
    image_field = ImageAllFieldWithMask(detections_path=args.features_path, max_detections=49, load_in_tmp=False)
    # Pipeline for text
    text_field = TextField(init_token='<bos>', eos_token='<eos>', lower=True, tokenize='spacy', remove_punctuation=True,
                           nopoints=False)

    # Create the dataset
    dataset = COCO(image_field, text_field, '../../datasets/COCO2014', args.annotation_folder, args.annotation_folder)
    ...
    model = Transformer(text_field.vocab.stoi['<bos>'], encoder, decoder,args).to(device)
    ...
    def lambda_lr_rl(s):
        ...
        if s <= refine_epoch:
            lr = args.rl_base_lr
        elif s <= refine_epoch + 4:
            lr = args.rl_base_lr * 0.2
        elif s <= refine_epoch + 8:
            lr = args.rl_base_lr * 0.2 * 0.2
        elif s <= refine_epoch + 12:
            lr = args.rl_base_lr * 0.2 * 0.2 * 0.2
        else:
            lr = args.rl_base_lr * 0.2 * 0.2 * 0.2 * 0.2
        return lr
    ...
    if args.resume_last or args.resume_best:
        ...
        if os.path.exists(fname):  #加载上一次或者最好的.pt
